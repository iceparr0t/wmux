...
- `wmux-hooks install prime-agent` writes an auto-loaded managed extension under `~/.prime/agent/extensions` and preserves an unmanaged extension at the managed path.
  ...
  New POSIX pane processes export `HERDR_WORKSPACE_ID`/`HERDR_TAB_ID`/`HERDR_PANE_ID` compatibility aliases because Prime Agent 0.7.1 forwards only that allowlist into daemon session extension-load scopes; `HERDR_ENV` and a Herdr socket are deliberately not set.
  The extension captures that session-bound identity at load, falls back to `WMUX_*` only outside daemon workers, and fails closed when identity is missing or malformed.
  Because Prime creates its persistent IPython kernel before applying the daemon session exec environment, the extension also pins the bound W/T/P IDs into each Python or `%%bash` IPython tool call so wmux commands cannot inherit the daemon's launch pane.
