...
The gold `?` indicator is reserved for a positively identified explicit input request. The managed Prime Agent extension marks an active `questionnaire` tool as waiting, keeps that state across overlapping root or nested questions, and returns to Working after the last questionnaire resolves; it does not guess that ordinary idle completion requires input.
New POSIX pane processes export `HERDR_WORKSPACE_ID`, `HERDR_TAB_ID`, and `HERDR_PANE_ID` compatibility aliases because Prime Agent 0.8.0 forwards that allowlist into each daemon session scope; wmux deliberately does not set `HERDR_ENV` or a Herdr socket.
...
Prime Agent 0.8.0 does not pin the scoped client environment into an already-created persistent IPython kernel. The extension does not inspect durable worker descriptors; it pins its captured tuple into each Python or `%%bash` tool call and clears stale W/T/P variables when no complete tuple was captured.
